...
const ensureManagedSessionAccess = async ({
	...
	if (!session) {
		throw new TRPCError({
			code: "FORBIDDEN",
			message: "You can only manage attendance for sessions assigned to you.",
		});
	}
...
export const attendanceRouter = createTRPCRouter({
	getRecentAttendance: teacherProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const sessions = await ctx.db.session.findMany({
				where: {
					attendances: { some: {} },
					...buildManagedSessionWhere(ctx.userRole, ctx.auth.userId),
				},
				include: {
			...
			return sessions.map((session) => {
				...
				const excused = session.attendances.filter(
					(a) => a.status === "EXCUSED",
				).length;
				return {
					id: session.id,
	...
	getAttendanceSessions: teacherProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			return sessions.map((session) => ({
				...
				excused: session.attendances.filter((a) => a.status === "EXCUSED")
					.length,
			}));
		}),
	...
	getSessionAttendance: teacherProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			return {
				...
				totalPresent: attendanceRecords.filter((a) => a.status === "PRESENT")
					.length,
				totalAbsent: attendanceRecords.filter((a) => a.status === "ABSENT")
					.length,
				totalExcused: attendanceRecords.filter((a) => a.status === "EXCUSED")
					.length,
			};
		}),
	...
	getAllStudentsWithAttendance: teacherProcedure.query(async ({ ctx }) => {
		...
		return students.map((student) => ({
			...
			presentCount: student.attendances.filter((a) => a.status === "PRESENT")
				.length,
			absentCount: student.attendances.filter((a) => a.status === "ABSENT")
				.length,
			excusedCount: student.attendances.filter((a) => a.status === "EXCUSED")
				.length,
			attendanceRate:
		...
		return sessions
			.filter((session) => {
				const bookedUserIds = new Set(
					session.seatBookings.map((b) => b.userId),
				);
