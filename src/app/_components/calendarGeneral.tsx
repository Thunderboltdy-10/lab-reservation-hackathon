...
const CalendarGeneral = () => {
	...
	const { data: myBookings, refetch: refetchBookings } =
		api.account.getMyBookings.useQuery(undefined, {
			enabled: !!data,
		});
					...
					<div className="stagger-children space-y-2">
						{data?.map((sess) => {
							const myBooking = myBookings?.find(
								(b) => b.session.id === sess.id,
							);
							const href =
								sess.lab.name.toLowerCase().includes("physics") ||
								sess.lab.name.toLowerCase().includes("chemistry")
									? "/book/physics"
									: "/book/biology";
							const totalSeats = getTotalSeats(
								parseLabConfig(sess.lab.defaultRowConfig),
							);
							...
							return (
								<div key={sess.id}>
									<div className="relative w-full rounded-lg border bg-card p-3 text-center transition-all hover:border-primary/50">
										...
										<div className="mt-2 mb-3 flex flex-wrap items-center justify-center gap-3 text-muted-foreground text-sm">
