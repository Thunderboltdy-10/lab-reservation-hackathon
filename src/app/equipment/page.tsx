...
export default function EquipmentPage() {
	...
	const { data: physicsLabData } = api.account.getLabId.useQuery({
		lab: "physics",
	});
	const { data: biologyLabData } = api.account.getLabId.useQuery({
		lab: "biology",
	});
	...
	const labOptions = useMemo(() => {
		return [physicsLabOption, biologyLabOption].filter(
	...
	const { data: categories, refetch: refetchCategories } =
		api.account.getEquipmentCategories.useQuery();
	const { data: brandOptions, refetch: refetchBrands } =
		api.account.getEquipmentBrands.useQuery();
	const createCategoryMutation =
		api.account.createEquipmentCategory.useMutation({
			onSuccess: (data) => {
	...
	const deleteCategoryMutation =
		api.account.deleteEquipmentCategory.useMutation({
			onSuccess: () => {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: Reset pagination whenever filters or lab selection change.
	useEffect(() => {
		setVisibleCount(20);
	...
	const brands = useMemo(() => {
		if (!equipment) return [];
		return Array.from(
			new Set(equipment.map((e) => e.brand).filter(Boolean)),
		).sort() as string[];
	}, [equipment]);
	...
	const filteredEquipment = useMemo(() => {
		...
		return equipment.filter((item) => {
			...
			const matchesCategory =
				categoryFilter === "ALL" ||
				(item.category?.name || "General") === categoryFilter;
			...
			<div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
				<div>
					...
					<p className="mt-2 text-muted-foreground">
						Manage and track all laboratory equipment and chemicals across
						facilities.
					</p>
				</div>
				<Dialog
				...
				>
					...
					<DialogContent className="rounded-2xl sm:max-w-[600px]">
						<DialogHeader>
							...
							<DialogDescription>
								Fill out the details below to log a new item into the inventory
								system.
							</DialogDescription>
						</DialogHeader>
						<div className="grid grid-cols-1 gap-4 space-y-4 py-4 md:grid-cols-2">
							...
							<div className="space-y-2">
								...
								<Select
									value={newUnitType}
									onValueChange={(value) => setNewUnitType(value as UnitType)}
								>
					...
					<AlertDialogContent className="rounded-2xl sm:max-w-[425px]">
						<AlertDialogHeader>
							...
							<AlertDialogDescription className="pt-2 text-base">
								...
								{categoryToDelete &&
									categoryToDelete.equipmentCount !== undefined &&
									categoryToDelete.equipmentCount > 0 && (
										<div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-amber-700 text-sm dark:text-amber-400">
											<strong>Warning:</strong>{" "}
											{categoryToDelete.equipmentCount} equipment{" "}
											{categoryToDelete.equipmentCount === 1 ? "item" : "items"}{" "}
											currently use this category. They will be set to have no
											category.
										</div>
									)}
							</AlertDialogDescription>
					...
					<AlertDialogContent className="rounded-2xl sm:max-w-[425px]">
						<AlertDialogHeader>
							...
							<AlertDialogDescription className="pt-2 text-base">
								...
								"?
								{brandToDelete &&
									brandToDelete.equipmentCount !== undefined &&
									brandToDelete.equipmentCount > 0 && (
										<div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-amber-700 text-sm dark:text-amber-400">
											<strong>Warning:</strong> {brandToDelete.equipmentCount}{" "}
											equipment{" "}
											{brandToDelete.equipmentCount === 1 ? "item" : "items"}{" "}
											currently use this brand. They will be set to have no
											brand.
										</div>
									)}
							</AlertDialogDescription>
			...
			<div className="mb-8 border-border/50 border-b pb-6">
				<div className="flex flex-col items-end gap-6 lg:flex-row">
					<div className="w-full flex-1 space-y-2">
						<Label htmlFor="lab-select" className="font-medium text-sm">
							Lab
						</Label>
						<Select value={selectedLab} onValueChange={setSelectedLab}>
							<SelectTrigger className="h-11 rounded-xl border-transparent bg-background/50 shadow-sm transition-colors hover:border-border">
					...
					<div className="w-full flex-[2] space-y-2">
						...
						<div className="relative">
							...
							<Input
								...
								className="h-11 rounded-xl border-transparent bg-background/50 pl-10 shadow-sm transition-colors hover:border-border"
								value={searchQuery}
						...
						<Select value={categoryFilter} onValueChange={setCategoryFilter}>
							...
							<SelectContent className="rounded-xl">
								...
								{categoryFilterOptions
									.filter((c) => !categories?.some((db) => db.name === c))
									.map((cat) => (
									...
									<td colSpan={6} className="px-6 py-16 text-center">
										<div className="flex flex-col items-center justify-center text-muted-foreground">
											...
											<p className="text-sm opacity-80">
												Try adjusting your filters or adding new equipment.
											</p>
										</div>
									</td>
								</tr>
									...
									return (
										...
										>
											...
											<td className="px-6 py-4">
												<Badge
													variant="secondary"
													className="mb-1 whitespace-nowrap rounded-lg font-normal"
												>
												...
												{item.expirationDate ? (
													<div className="flex flex-col items-start gap-1.5">
														...
														{expStatus && (
															<Badge
																variant={
																	expStatus.urgent ? "destructive" : "outline"
																}
																className="rounded-md px-1.5 font-normal text-[10px]"
															>
																{expStatus.urgent && (
