...
export default clerkMiddleware(async (auth, req) => {
	if (!isPublicRoute(req)) {
		...
		if (role === "STUDENT") {
			if (
				new RegExp(adminOnlyRoutes).test(req.nextUrl.pathname) ||
				new RegExp(teacherOnlyRoutes).test(req.nextUrl.pathname)
			)
				throw new Error("You do not have access to this page");
		} else if (role === "TEACHER") {
			if (
				new RegExp(adminOnlyRoutes).test(req.nextUrl.pathname) ||
				new RegExp(studentOnlyRoutes).test(req.nextUrl.pathname)
			)
				throw new Error("You do not have access to this page");
		}
	}
